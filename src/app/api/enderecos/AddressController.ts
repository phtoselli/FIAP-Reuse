...
export class AddressController {
  ...
  async updateAddress(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
      ...
      const enderecoAtualizado = await this.addressService.updateAddress(id, updateData, userId);
   ...
   */
  async countAddresses(request: NextRequest, { params }: { params: { tipo: string } }) {
    try {
      const { tipo } = params;
      const { searchParams } = new URL(request.url);
      ...
      switch (tipo) {
        ...
        default:
          return NextResponse.json(
            { error: 'Tipo de contagem inválido. Use: usuario, cidade ou estado' },
            { status: 400 }
          );
      }
