...
import { ProductModel, ProductCreateModel, ProductUpdateModel } from '@/types/product/ProductModel';
...
export class ProductService {
   ...
   */
  async getProductsByCategory(categoryId: string, activeOnly: boolean = true): Promise<ProductModel[]> {
    try {
      const posts = await this.postRepository.findByCategory(categoryId, activeOnly);
   ...
   */
  async getProductsWithoutCategoryFilter(activeOnly: boolean = true, limit?: number, offset?: number): Promise<{
    produtos: ProductModel[];
    total: number;
    limit: number;
    offset: number;
    ...
    try {
      ...
      throw new Error('Erro interno ao listar produtos sem filtro de categoria');
    }
  }
   ...
   */
  async updateProduct(id: string, productData: ProductUpdateModel): Promise<ProductModel> {
    try {
      ...
      if (productData.descricao !== undefined) updateData.description = productData.descricao;
      if (productData.imagemUrl !== undefined) updateData.imageUrl = productData.imagemUrl;
      if (productData.categoriaId) updateData.categoryId = productData.categoriaId;
      if (productData.subcategoriaId) updateData.subcategoryId = productData.subcategoriaId;
      if (productData.condicaoId !== undefined) updateData.conditionId = productData.condicaoId;
      if (productData.ativo !== undefined) updateData.isActive = productData.ativo;
      if (productData.avaliacao !== undefined) updateData.rating = productData.avaliacao;
