	...
	async acceptProposal(
		...
		try {
			...
			if (existingProposal.responderId !== responderId) {
				console.log(
					"🔍 Service - ResponderId não confere:",
					existingProposal.responderId,
					"vs",
					responderId
				);
	...
	async finalizeShippingDetails(
		...
		try {
			...
			if (existingProposal.status !== "accepted") {
				throw new Error(
					"Apenas propostas aceitas podem ter detalhes de envio finalizados"
				);
			}
	...
	private mapProposalToResponseModel(proposal: any): ProposalResponseModel {
		...
		try {
			const result = {
				...
				totalItems:
					proposal.items?.filter((item: any) => item.isOffered).length || 0,
			};
